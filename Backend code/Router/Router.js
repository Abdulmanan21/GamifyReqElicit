...
router.post(
  ...
  [
    ...
    body("Desc", "Enter a valid descr").isLength({ min: 5 }),
  ],
  async (req, res) => {
...
let gam1 = await Gamify.findOne({user:req.user.id});


if(gam1){
  if((gam1.point+10)>=250  ){
    gam1=Object.assign(gam1,{point:0,Level:'No Level',Ranks:gam1.Ranks+1})
    }else if((gam1.point+10)>=200){
    gam1=Object.assign(gam1,{point:gam1.point+10,Level:'Three'})
    }else if((gam1.point+10)>=150 && (gam1.point+10)<200){
      gam1=Object.assign(gam1,{point:gam1.point+10,Level:'Two'})
   
    }else if((gam1.point+10)>=100 && (gam1.point+10)<150){
      gam1=Object.assign(gam1,{point:gam1.point+10,Level:'One'})
   
    }
    else{ gam1=Object.assign(gam1,{point:gam1.point+10,Level:'No Level'})
  }
  gam1 = await Gamify.findByIdAndUpdate(
    gam1.id,
    { $set: gam1 }
    )
 
}else{
  gam1= await Gamify.create({
    user:req.user.id,
    point:10
})
